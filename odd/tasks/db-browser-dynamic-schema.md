...
- [ ] T6 Simpler filter, automatic reload (user decision 2026-09-24): no search or refresh buttons. The filter applies as you type (debounced; Enter immediate). Rows and schema reload on their own when the app announces a change: batch progress/completion (`batchStore`), document/page import or deletion (document-explorer events), sync applied (`syncStore`); coalesced, keeping page, sort and filter. Writes from outside the app are picked up on re-entering the view, as today.
- [ ] T7 Fixed header on scroll (user request 2026-09-24): page header, table toolbar (row count, JSON/CSV, rows per page) and column names stay visible; only the rows scroll. Approach: the table card fills the remaining height and scrolls itself, with sticky column headers inside it. Queued until T6 (same file) lands.
