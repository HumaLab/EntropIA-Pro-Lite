...
- [x] T6c Polish from the user's review (inline, one commit each): panels at `color-mix(surface-raised 78%)` (`089a302a`); sync line removed from Estado del corpus, since the status bar already shows it; corpus footer only when something is pending; indicator gap --space-5 (`904073c4`); link alpha by page luminance via `linkAlphaForPage` (`f09df834`), then raised to 0.32 dark / 0.6 light (`417784bf`); Inicio header at `color-mix(surface-app 78%)` (`home-view__header`). Each change RED then GREEN; desktop 160 files / 2102; typecheck Pro+Lite, lint, format:check clean.
  - Note: the header is sticky; on a short Inicio nothing scrolls under it, but if the page ever scrolls, content will show faintly through it.
- All tasks done (T1–T6c). The user confirmed everything in the running app (2026-09-23) and asked for commit + push. Feature closed.
- [x] T7 Corpus unit (user request 2026-09-24): OCR, STT, Texto and Embeddings count viewable files (PDF pages, images, audios), not documents. OCR = pages+images with OCR / scanned pages (no native text) + images; STT = transcribed audios / audios; Texto = files with text / pages+images+audios; Embeddings = files with text and a vector / files with text. Documentos keeps counting documents.
  - Delegated: `0421d451` (store: `getCorpusStats` per viewable asset, new `textUniverse`; one statement, no ';'), `863d1e73` (Texto shows text / textUniverse). RED store 3, desktop 1; GREEN store 296, desktop 2112; typecheck Pro+Lite, lint, format:check clean.
  - Parent check on the live archive (read-only, the exact SQL): OCR 618 / 4129, STT 12 / 14, Texto 630 / 4143 (= 618 + 12 over 4129 + 14), Embeddings 630 / 630. next T5 (Nueva investigación, Nuevo documento, Crear colección, Ver guía), then T6 (constellation, needs a decision).
- [x] T8 Actividad reciente titled above its table like Acceso rápido (`home-view__section-label`), first column "Documento" (`home.activity.columnDocument`), grow moved to the new section so the page layout is unchanged (inline, `e3ad2a4d`). RED 1; GREEN desktop 2113; typecheck Pro+Lite, lint, format:check clean.
- [x] T9 Header actions (user requests 2026-09-24, inline): the research button became a new-chat action (`d4da9258`: `ragChat.initialize()` then `startNew()` then open Chat, so the restore on first initialize cannot undo the fresh conversation); labels renamed to "Importar", "Recuperar", "Escribir" (en Import, Retrieve, Write) — the first-run block's import button shares the key; the import dialog title keeps "Importar fuentes". RED 3 then 7; GREEN desktop 2142; typecheck, lint, format:check clean.
- [x] T10 Continuar shows up to 4 entries (user request 2026-09-24; the screenshot measured ~69px per row and ~74px free under the third). One shared `CONTINUAR_LIMIT = 4` (lib/home.ts) drives both the view's slice and the word-count slots. RED 2; GREEN desktop 2142; typecheck, lint, format:check clean.
- [x] T11 TopBar: a Colecciones icon (`folder`) first among the section icons (user request 2026-09-24, inline). It resets the path to Inicio > Colecciones (`resetToPath`) rather than `openRootSection`, which would append the list after the current collection/document. RED 1; GREEN desktop 2143; typecheck Pro+Lite, lint, format:check clean.
- [x] T12 TopBar: an Inicio icon (Tabler `home`, new in ActionIcon) first among the section icons, before Colecciones (user request 2026-09-24, inline). It pushes home like any section (browser-like history). The EntropIA mark was the fallback the user offered; the house reads as "start page" and the mark already sits beside the product name. RED ui 5 + desktop 2; GREEN ui 787, desktop 2201; typecheck Pro+Lite, lint, format:check clean.
