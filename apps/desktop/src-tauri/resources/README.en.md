...
3. Push tag `v*` → **Release** builds the lean installer (NSIS) with the manifest URL + public key baked into the binary. `build.rs` fails closed if a release build embeds the fixture without a baked source, so an installer that cannot download the runtime is never published.
