...
describe('pane-scoped ids in the RAG chat view', () => {
  ...
  it('suffixes the rag-chat title id and its aria-labelledby reference with the pane id', () => {
    expect(SOURCE).toMatch(/aria-labelledby="rag-chat-title-\{paneId\}"/)
    expect(SOURCE).toMatch(/id="rag-chat-title-\{paneId\}"/)
  })

  it('leaves no unsuffixed literal id or reference behind', () => {
    expect(SOURCE).not.toMatch(/id="rag-chat-title"/)
    expect(SOURCE).not.toMatch(/aria-labelledby="rag-chat-title"/)
  })
})
