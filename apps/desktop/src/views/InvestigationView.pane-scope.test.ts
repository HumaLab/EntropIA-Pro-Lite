...
describe('pane-scoped ids in the investigation view', () => {
  ...
  it('suffixes the investigation title id and its aria-labelledby reference with the pane id', () => {
    expect(SOURCE).toMatch(/aria-labelledby="investigation-title-\{paneId\}"/)
    expect(SOURCE).toMatch(/id="investigation-title-\{paneId\}"/)
  })

  it('leaves no unsuffixed literal id or reference behind', () => {
    expect(SOURCE).not.toMatch(/id="investigation-title"/)
    expect(SOURCE).not.toMatch(/aria-labelledby="investigation-title"/)
  })
})
