...
| §27.3 — procesador CSL | `hayagriva` 0.10.1 en Rust (MIT OR Apache-2.0). citeproc-js es CPAL-1.0 OR AGPL-1.0 y `@citation-js/plugin-csl` depende de él pese a declararse MIT | Incompatible con la licencia MIT del proyecto. Sujeto al spike S3: si hayagriva no cubre clústeres, se eleva antes de fijar §9.5 |
...
- [ ] S1 Tiptap y JSON canónico: montar `@tiptap/extension-table@2.26.4` y `tiptap-footnotes@2.0.4` junto con un nodo propio mínimo; verificar ida y vuelta de `getJSON`/`setContent` sin pérdida, con `schema_version` explícito y una extensión desconocida presente. Medir apertura y edición sobre un manuscrito de 30.000 palabras con centenares de nodos.
- [ ] S2 Anchors: probar transformación de rangos bajo transacciones ProseMirror con división y fusión de bloques, movimiento, borrado del objetivo y undo/redo. Registrar qué casos quedan sin resolución inequívoca; §8.5 exige marcarlos desactualizados, no reubicarlos por coincidencia textual.
- [ ] S3 CSL con `hayagriva` 0.10.1: renderizar un clúster de dos items con locator, tipo de locator, prefijo, sufijo y supresión de autor; reordenar el clúster; cambiar de APA a Chicago y volver a renderizar; generar bibliografía solo con las obras citadas. Si la biblioteca no cubre este conjunto, elevar antes de fijar §9.5 y no sustituirla por citeproc-js sin resolver la incompatibilidad de licencia documentada en §29.1.
- [ ] S4 DOCX: construir el documento patrón de §17.4 con jerarquía de títulos, énfasis, listas, citas en bloque, enlaces, tablas, notas al pie, citas documentales, clústeres Zotero y bibliografía. Exportarlo y abrirlo en aplicaciones reales; registrar versiones verificadas y completar la matriz de fidelidad distinguiendo soporte nativo, representación alternativa y elemento no admitido. `html-docx-js` no tiene modelo de objetos DOCX (G7): si no supera la prueba, elegir reemplazo en esta unidad.
- [ ] S5 Zotero local: contra una instalación real, probar biblioteca accesible, API deshabilitada (`403`), endpoint ausente, timeout, identidad de instancia por `Zotero-Server-ID` y comportamiento sin límite explícito de consulta. Confirmar que el acceso no depende de `local-ml`. El cliente existente en `especialistas/zotero.rs` del crate `entropia-agent` sirve como evidencia de que el enfoque funciona, no como implementación reutilizable.
- [ ] S6 Journal: elegir entre tabla de recuperación en SQLite y archivo con escritura atómica; medir el plazo máximo entre persistencias durante escritura continua y la ventana de pérdida real bajo terminación forzada del proceso. Distinguir explícitamente la garantía ante muerte del proceso de la garantía ante corte eléctrico, como hace `plan-lote.md §13`.
...
- [ ] Reutilizar `FtsSearchController` y `similarAssets`; no implementar un segundo motor de recuperación.
- [ ] Resolver la brecha de direccionamiento de G3: el visor superpone geometría en píxeles y la procedencia usa offsets de caracteres. Definir el mapeo o declarar explícitamente el alcance del resaltado.
- [ ] Insertar cita creando nodo, proyección y evento de procedencia en una sola transacción. Ante fallo, conservar el borrador y el estado de error, sin registros parcialmente confirmados.
- [ ] Implementar apertura de la fuente con los cinco pasos de §10.2, incluida la degradación a fragmento y metadatos cuando el anclaje ya no resuelve.
- [ ] Probar copia, pegado, duplicación, movimiento, borrado, undo/redo y restauración sin desincronizar JSON, proyecciones y procedencia. Copiar crea identidades nuevas conservando la referencia a la fuente; mover conserva identidad.
- [ ] Implementar la política de borrado de assets referenciados según §29.1 y advertir dependencias antes de confirmar.
...
- [ ] Conector local de solo lectura con timeout, límites y paginación explícitos.
- [ ] Caché regenerable particionada por origen e instancia. Un cambio de instancia invalida la caché, no los snapshots del manuscrito, y nunca revincula citas por coincidencia de clave.
- [ ] Estados de §11.3 sin diagnósticos falsos: no afirmar que Zotero está cerrado o instalado sin evidencia.
- [ ] Clústeres con locator, prefijo, sufijo, supresión de autor y reordenamiento; almacenar datos equivalentes a CSL, no la cadena renderizada.
- [ ] Estilos CSL con conjunto reducido inicial y ruta segura para incorporar otros archivos, validados antes de usarse.
- [ ] Bibliografía dinámica solo con obras citadas, como vista derivada.
- [ ] Funcionamiento degradado con Zotero cerrado, renderizando desde snapshots y marcando pendiente de verificación.
...
- [ ] Publicar la matriz real de capacidades por variante y proveedor antes de exponer acciones. Las no disponibles se muestran como tales; ninguna ausencia bloquea la edición manual.
- [ ] Constructor de contexto con límites explícitos, deduplicación, vista previa de lo que se envía y registro de las piezas enviadas.
- [ ] Sugerencias no destructivas con las tres acciones de §14.2 y distinción entre evidencia del corpus, metadatos Zotero y consulta efectiva de texto.
- [ ] Validar el rango objetivo con anchors y hash del contenido objetivo, no del manuscrito completo. Aplicación y registro de aceptación en una sola operación persistente; repetir una aceptación no duplica texto.
- [ ] Registrar procedencia de cada operación de IA con texto original, texto aceptado, acción, fecha, modelo, proveedor y evidencia.
