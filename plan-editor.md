...
**Estado:** el motor está completo y probado (Rust, 63 tests) **y la aplicación ya lo alcanza**: cinco comandos Tauri lo exponen y la pestaña Zotero sonda, lee la biblioteca por páginas, filtra e inserta una cita. `writing_zotero_probe` **nunca falla**: un Zotero inalcanzable es un estado que se reporta, no un error — devolver `Err` ahí haría parecer rota la interfaz en vez de ausente la biblioteca. **Falta aún:** `WritingCitationDialog.svelte` (locator, afijos y supresión por cita), escribir la proyección `writing_zotero_citations` en el guardado — la tabla existe desde la Unidad 1 y nada la escribe todavía — y re-renderizar las citas del manuscrito al cambiar de estilo. Los criterios 12 a 17 **siguen sin poder verificarse de punta a punta** hasta que eso exista, y necesitan Zotero corriendo.
