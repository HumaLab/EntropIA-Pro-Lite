...
- [~] S4 DOCX — **mitad estructural resuelta; falta la revisión visual del usuario**. Documento patrón construido y exportado por dos candidatos en `docs/escritura-docx-spike/`. `html-docx-js` **no genera un DOCX**: su `document.xml` son 2 KB de espacios de nombres envolviendo un `altChunk`, y el contenido real es un MHTML en `afchunk.mht`, así que lo que se ve lo produce el importador de la aplicación que abre. No tiene `footnotes.xml`, ni tabla, ni hipervínculo, ni estilos de título, ni numeración: **no puede cumplir §17.1**. El candidato `docx` 9.7.1 (MIT) los tiene todos y además emite `comments.xml`, lo que habilita §17.2. Alternativa Rust `docx-rs` 0.4.22 (MIT) a evaluar en la Unidad 8 para mantener DOCX junto a CSL. Falta abrir los archivos en procesadores reales y registrar aplicaciones y versiones, conforme a §17.4.
