...
**Estado:** completa de punta a punta. Motor en Rust (63 tests), cinco comandos Tauri, la pestaña Zotero, el diálogo de cita y la proyección `writing_zotero_citations` escrita en la misma transacción que el contenido. La casilla de supresión está redactada como la situación y no como el mecanismo — «*ya nombré al autor en mi frase*» dice cuándo tildarla; «suprimir autor» solo dice qué hace el software — y cuando el estilo no lo permite **se dice en voz alta** en vez de dejar una casilla que no hace nada. **Falta verificación con Zotero corriendo**: los criterios 12 a 17 necesitan una biblioteca real y el ojo del usuario, y eso no lo puede hacer el agente.
