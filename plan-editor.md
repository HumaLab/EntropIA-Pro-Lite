...
- [ ] Clústeres con locator, prefijo, sufijo, supresión de autor y reordenamiento; almacenar datos equivalentes a CSL, no la cadena renderizada. **Parcial:** el helper de afijos está hecho y probado (`writing/csl/affix.rs`, 9 tests) — S3 advirtió que "esto es nuestro código haciendo el trabajo de CSL" y exigía un caso por clase de estilo: los estilos con corchetes reciben los afijos **adentro**, los de nota envuelven todo y **pierden su punto final** antes del sufijo. Falta el renderizado en sí. **hayagriva aún no es dependencia a propósito**: entra junto con el renderizador, donde su API se verifica contra un build y no contra una descripción. **Supresión de autor: resuelta por sustracción** (decidido por el usuario, 2026-09-16). hayagriva no la tiene, y sus dos aproximaciones descartan el locator. Pero sí sabe renderizar solo el autor, así que se renderiza dos veces y se resta: `(Ginzburg, 1976, p. 45) − Ginzburg → (1976, p. 45)`, **conservando la página**. Si el autor no aparece en el renderizado, se responde `None` y la cita completa queda — nunca una cita mutilada. `writing/csl/suppress.rs`, 11 tests.
