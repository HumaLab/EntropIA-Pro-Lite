...
- [ ] Estilos CSL con conjunto reducido inicial y ruta segura para incorporar otros archivos, validados antes de usarse. **Parcial:** el conjunto inicial es el archivo empaquetado de hayagriva (feature `archive`), así que una instalación nueva renderiza sin descargar nada, y **un estilo desconocido se rechaza por nombre en vez de caer a APA** — caer silenciosamente mal-renderizaría un manuscrito entero. Falta la ruta para cargar archivos `.csl` propios con validación previa.
