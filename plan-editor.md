...
Su alcance está acotado por G9: el trait `ClienteLlm` permite sustituir el proveedor de chat en el sitio de llamada, pero `ClienteEmbeddings` y `ClienteRerank` son structs concretos atados a OpenRouter.

**Corrección (2026-09-17).** Este documento leyó ese límite de más. G9 dice que el **proveedor de embeddings no se puede sustituir**, no que la recuperación no funcione: con credencial OpenRouter funciona, y es el mismo camino que ya usa Chat/RAG. Las cuatro acciones de evidencia quedaron en gris en máquinas donde la recuperación andaba, por dos motivos distintos — `hasRetrieval` nunca se cableó y tomaba su valor por defecto, y no existía forma de traer pasajes sin generar una respuesta. Ambos resueltos abajo. El límite real que G9 sí impone sigue en pie: sin OpenRouter no hay recuperación, y la matriz lo dice.

- [x] Publicar la matriz real de capacidades por variante y proveedor antes de exponer acciones. Las no disponibles se muestran como tales; ninguna ausencia bloquea la edición manual. `writing/agent_actions.rs`: las catorce acciones de §14.1 contra lo que cada una necesita. Una acción que no puede correr **se lista con su motivo**, ni oculta ni ofrecida para fallar cuando el autor ya eligió un pasaje; y una matriz ilegible no ofrece nada en vez de todo. 6 tests.

  **Las cuatro de recuperación ya no están en gris con credencial** (2026-09-17). Hacía falta un comando propio: `rag_ask` recupera, reordena, arma un prompt y genera una respuesta, y estas acciones no quieren nada de eso — quieren los pasajes, para que la propuesta se apoye en algo que el autor pueda abrir y verificar. Pedirle a `rag_ask` y tirar su respuesta habría pagado una generación que nadie lee.

  `writing_corpus_retrieve` reusa la recuperación que ya existe —las mismas piernas híbridas, los mismos parámetros de los mismos settings— y **se detiene antes de la parte que cuesta una llamada al modelo**. El reordenamiento se saltea a propósito: es una llamada al proveedor por recuperación, y estas acciones se aprietan mientras se escribe. Los pasajes entran al contexto como piezas `corpus` **con su `asset_id`**, que es lo que hace que `evidenceOf` los cuente como evidencia y no como prosa del modelo — sin esa identidad las mismas palabras llegan como algo que dijo el autor. 18 tests.
...
**Verificado en la aplicación (2026-09-17):** la matriz se publica bien, el viaje completo funciona —pedir, recibir la propuesta, aceptarla, y que una segunda aceptación no inserte nada— y la vista previa dejó de ser un botón. Ese botón armaba el contexto *antes* de enviar, pero como una acción prepara y envía en un solo clic, la vista aparecía siempre después: no era una promesa sino un registro, y mostraba el pasaje que el autor acababa de seleccionar y estaba viendo resaltado. Se eliminó y el panel dice lo que ahora es. El día que el contexto crezca más allá de la selección, mirar antes de enviar vuelve a merecer un control.
