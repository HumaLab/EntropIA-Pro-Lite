...
**Estado:** cuatro de cinco ítems cerrados. **Falta la llamada al modelo**, y no por descuido: `research_request` es un motor de trabajos del modo investigación y `llm_ask` está atado a una colección y responde por cola — ninguno tiene la forma que necesita una acción de escritura, que es *un pasaje y su contexto entran, una propuesta sale*. Forzar cualquiera de los dos habría dejado algo que parece conectado y no lo está. Hace falta un comando delgado propio, y con él el último ítem (registrar la procedencia de cada operación de IA, que ya tiene su tabla y su vocabulario). La pestaña lo dice en pantalla en vez de simularlo.
