...
**Estado:** los siete ítems están implementados y probados **en el motor** (Rust: conector, caché, CSL, afijos, supresión, degradado, estilos — 63 tests). **Pero nada en la aplicación lo alcanza todavía:** faltan los comandos Tauri que lo expongan y las dos vistas del panel (`WritingZoteroTab.svelte`, `WritingCitationDialog.svelte`). Hasta que existan, la pestaña Zotero sigue mostrando su texto de pendiente y los criterios 12 a 17 **no pueden verificarse**, porque no hay por dónde ejercitarlos. Un tilde por ítem no equivale a una unidad utilizable.
